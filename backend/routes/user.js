...
router.post('/signup', async (req, res) => {
    const { name, email, password, phone, streetName, city, province, country, postalCode } = req.body;
...
router.put('/users/:id', async (req, res) => {
    ...
    try {
        const updatedUser = await User.findByIdAndUpdate(id, { name, address }, { new: true });
        if (!updatedUser) {
            return res.status(404).json({ error: 'User not found' });
        }
